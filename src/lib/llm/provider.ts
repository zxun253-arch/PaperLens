...
async function requestJson<T>(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(sanitizeErrorText(formatHttpError(response.status, data), apiKey));
    }
    ...
    if (error instanceof TypeError) {
      throw new Error("网络请求失败，请检查网络连接、Base URL、服务商接口或桌面环境访问限制。");
    }
...
function formatHttpError(status: number, data: unknown) {
  ...
  if (status === 401 || status === 403) return `鉴权失败，请检查 API Key 或权限${suffix}`;
  if (status === 404) return `接口地址或模型不存在，请检查 Base URL 和 Model Name${suffix}`;
...
async function callGemini(
  ...
  const prompt = [
    system ? `系统要求：\n${system}` : "",
    ...messages.map((message) => `${message.role === "assistant" ? "助手" : "用户"}：\n${message.content}`),
  ]
    .filter(Boolean)
    .join("\n\n");
  const raw = await requestJson<unknown>(
...
export async function callLLM(
  ...
  if (config.adapter === "placeholder") {
    throw new Error("该服务商当前暂未完整实现，请使用 OpenAI-compatible 通用接口或等待后续支持。");
  }
...
export async function testAiConnection(): Promise<{
  ...
  try {
    ...
    return {
      ok: false,
      message: error instanceof Error ? sanitizeErrorText(error.message, settings.apiKey) : "测试连接失败。",
    };
  }
}
